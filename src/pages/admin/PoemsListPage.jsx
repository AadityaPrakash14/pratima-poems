 ...
 * 
 * Features:
 * - List all poems with status, category, featured, updated date
...
export default function PoemsListPage() {
  ...
  // Load poems and categories
  const loadData = useCallback(async () => {
    try {
      // Load poems with filters
      const poemOptions = {};
      if (statusFilter) {
        poemOptions.status = statusFilter;
      }
      if (categoryFilter) {
        poemOptions.categoryId = categoryFilter;
      }

      const [poemsData, categoriesData] = await Promise.all([
        listPoems(poemOptions),
        listCategories({ type: 'poem' }),
      ]);
      ...
      setError(err.message || 'कविताएँ लोड नहीं हो सकीं।');
    }
  ...
  const handleEditPoem = (poem) => {
    navigate(`/admin/poems/${poem.id}/edit`);
  };

  // Status transition handlers
  const handlePublish = async (poem, e) => {
    e?.stopPropagation();
    setActionLoadingId(poem.id);
    try {
      const updated = await publishPoem(poem.id);
      setPoems(prev => prev.map(p => p.id === poem.id ? updated : p));
    } catch (err) {
  ...
  const handleArchive = async (poem, e) => {
    e?.stopPropagation();
    setActionLoadingId(poem.id);
    try {
      const updated = await archivePoem(poem.id);
      setPoems(prev => prev.map(p => p.id === poem.id ? updated : p));
    } catch (err) {
  ...
  // Delete handlers
  const handleDeleteClick = (poem, e) => {
    e?.stopPropagation();
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      alert(err.message || 'कविता हटाई नहीं जा सकी।');
    } finally {
      setIsDeleting(false);
    }
  };
  ...
  const renderContent = () => {
    if (loading) {
      return <AdminLoadingState message="कविताएँ लोड हो रही हैं..." />;
    }

    if (error) {
      return (
        <AdminErrorState
          title="कविताएँ लोड नहीं हो सकीं"
          message={error}
          onRetry={() => {
            setLoading(true);
            setError(null);
    ...
    if (poems.length === 0) {
      ...
      return (
        <AdminEmptyState
          icon={<PoemIcon className="w-full h-full" />}
          title={hasFilters ? 'कोई परिणाम नहीं' : 'कोई कविता नहीं है'}
          description={
            hasFilters 
              ? 'चयनित फ़िल्टर के अनुसार कोई कविता नहीं मिली। फ़िल्टर बदलकर देखें।'
              : 'अभी तक कोई कविता नहीं बनाई गई है। नई कविता बनाने के लिए "नई कविता" बटन पर क्लिक करें।'
          }
          action={!hasFilters ? {
  ...
  return (
    <div>
      ...
      <AdminPageHeader
        title="कविताएँ"
        description="सभी कविताओं का प्रबंधन करें"
        action={{
          label: 'नई कविता',
          onClick: handleNewPoem,
          icon: <PlusIcon />,
      ...
      >
        ...
        {!loading && !error && (
          <div 
            ...
            style={{ color: 'var(--color-muted)' }}
          >
            {poems.length} कविता{poems.length !== 1 ? 'एँ' : ''}
          </div>
        )}
      </div>
      ...
      <AdminConfirmDialog
        ...
        title="कविता हटाएँ?"
        message={`क्या आप "${deleteTarget?.title}" कविता को हटाना चाहते हैं? यह क्रिया पूर्ववत नहीं की जा सकती।`}
        confirmLabel="हटाएँ"
